...
// ✅ Tabbed: Customers | Complaints | Inquiries
// ✅ Staff can view photos, reply to complaints & inquiries, update status
// ✅ Store-scoped, reloads on store switch
// ✅ Customer portal account linking (assign branch)
...
import {
  ...
  Eye, Image as ImageIcon, ChevronDown, Filter, Reply, User,
  AlertTriangle, Package, Phone, Mail, MapPin, Edit2, Trash2,
...
type PageTab = 'customers' | 'complaints' | 'inquiries';
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            {complaint?.imageUrl && (
              <div>
                ...
                <div
                  className="relative rounded-xl overflow-hidden cursor-pointer border border-border group"
...
export default function CustomersPage() {
  ...
  const loadComplaints = useCallback(async () => {
    ...
    try {
      ...
      setComplaints((data || []).map(x => ({
        ...
        imageUrl: x.image_url ?? null, status: x.status,
  ...
  const loadInquiries = useCallback(async () => {
    ...
    try {
      let q = supabase.from('customer_inquiries').select('*').order('created_at', { ascending: false });
      if (currentStoreId) q = q.eq('store_id', currentStoreId);
      const { data, error } = await q;
      if (error) throw error;
      setInquiries((data || []).map(x => ({
        id: x.id, customerId: x.customer_id, customerName: x.customer_name,
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return customers.filter(c => `${c.firstName} ${c.lastName} ${c.email ?? ''} ${c.phone ?? ''}`.toLowerCase().includes(q));
  ...
  // ── Assign branch to unlinked customer portal user ──
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <Button
            ...
            size="sm"
            className="gap-1.5"
            onClick={() => { loadCustomers(); loadComplaints(); loadInquiries(); }}
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          { label: 'With Photos',     value: stats.withPhoto,                      color: 'text-purple-600',  icon: <ImageIcon className="w-4 h-4" /> },
        ].map((s, i) => (
      ...
      <div className="flex border-b border-border">
        {([
          { key: 'customers',  label: 'Customers',  icon: <Users className="w-4 h-4" />,         badge: customers.length },
          ...
          >
            ...
            {t.badge > 0 && (
              <span className={`text-xs px-1.5 py-0.5 rounded-full font-semibold ${
                (t as any).badgeColor || 'bg-muted text-muted-foreground'
              }`}>{t.badge}</span>
            )}
          </button>
        ))}
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="border-b border-border">
                      <tr>
                        {['Name', 'Contact', 'City', 'Total Spent', 'Points', 'Branch', 'Actions'].map(h => (
                        ...
                        <tr key={c.id} className="border-b border-border hover:bg-muted/40 transition-colors">
                          ...
                          <td className="py-3 px-3 text-center">
                            <div className="flex items-center justify-center gap-1">
                              ...
                              <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => handleDelete(c.id, `${c.firstName} ${c.lastName}`)}>
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <div className="flex gap-1.5 flex-wrap">
              {['all', 'open', 'in-progress', 'resolved', 'closed'].map(f => (
                ...
                >
                  {f === 'all' ? 'All' : COMPLAINT_STATUS[f]?.label}
          ...
          {loadingComplaints ? (
            ...
            <div className="space-y-3">
              {filteredComplaints.map(c => {
                const sc = COMPLAINT_STATUS[c.status] ?? COMPLAINT_STATUS.open;
                return (
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <div className="flex gap-1.5 flex-wrap">
              {['all', 'open', 'in-progress', 'resolved'].map(f => (
                ...
                >
                  {f === 'all' ? 'All' : INQUIRY_STATUS[f]?.label}
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-3 min-w-0 flex-1">
                          <div className="w-9 h-9 rounded-full bg-blue-50 text-blue-600 font-bold text-xs flex items-center justify-center shrink-0 mt-0.5">
                            {i.customerName.split(' ').map(n => n[0]).join('').slice(0,2)}
                          </div>
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 flex-wrap mb-1">
                              <p className="font-bold text-foreground text-sm">{i.subject}</p>
                              <span style={{ display:'inline-flex', padding:'2px 8px', borderRadius:100, background: pc.bg, color: pc.color, fontSize:10, fontWeight:600 }}>
                                {i.priority}
                              </span>
                              {!i.adminReply && (
          ...
          <div className="bg-background rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-3 overflow-y-auto">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[['city','City'],['state','State/County'],['zipCode','ZIP/Postal']].map(([k, label]) => (
                  <div key={k} className="space-y-1">
